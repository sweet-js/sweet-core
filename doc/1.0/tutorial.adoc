...
syntax cond = function (ctx) {
  ...
  for (let stx of bodyCtx) { // <2>
    if (isKeyword(stx) && unwrap(stx).value === 'case') { // <3>
...
----
<1> The `.contextify` method gives us another context inside the delimiter. In this case, that is everything inside of `{ ... }`.
<2> A macro context is iterable so you can `for-of` over it.
<3> Utility functions are available for import to help inspect what kind of syntax you are processing. Note the `for syntax` on the import declaration, which is required for the helpers to be available inside of a macro definition. Modules and macros are described more <<_sweet_modules, later>>.
