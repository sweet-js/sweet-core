...
s-expression, it is easy to implement \lstinline!if! as a
macro.
...
\lstinline!/x/g! as a regular expression or as division operators and
operands (as in \lstinline!4.0/x/g!). Surprisingly, this history
...
definitions difficult. For example, the \lstinline!if! statement in
...
correctly implement an \lstinline!if! macro but this is complicated by
...
and manipulate the partial AST. For example, an \lstinline!if! macro
...
implement the \lstinline!invoke! pattern class, described in Section
\ref{sec:invoke}, which allows macro authors to extend the patterns
used to match syntax.

\textbf{todo: more about the project like users}
...
the source string ``\lstinline!foo(/x/)!'' is lexed into the sequence
...
(analogous to \lstinline!syntax-rules! in Scheme \cite{Clinger1991})
and \emph{case} macros (analogous to \lstinline!syntax-case! in
...
so we use the \emph{pattern class} \lstinline!:expr! to match the
largest possible expression, so in this example the entire expression
\lstinline!40 + 2! is bound to \lstinline!$init!.
...
Our solution to this problem is the \lstinline!:invoke! pattern class.
...
By using \lstinline!:invoke!, macro writers can encode patterns
...
Since \lstinline!id 100! is not a valid expression, this will fail to
parse.
...
it is not guaranteed that a semicolon will close off a \lstinline!var!
statement.
