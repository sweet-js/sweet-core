...
\author{Tim Disney}
\date{\today}
\title{Sweet Hygienic Macros for Full ES5 JavaScript}
\hypersetup{
  pdfkeywords={},
...
\abstract{
Lisp and Scheme have demonstrated the power of macros to enable programmers to evolve and craft languages. In languages with more complex syntax, macros have had less success. In part, this has been due to the difficulty in building expressive hygienic macro systems for such languages.
JavaScript in particular presents unique challenges for macro systems due to ambiguities in the lexing stage that force the JavaScript lexer and parser to be intertwined.

In this paper we present a novel solution to the lexing ambiguity of JavaScript that enables us to cleanly separate the JavaScript lexer and parser. This separated lexer and parser allows us to implement an expressive hygienic macro system for JavaScript called sweet.js.
}
...
more powerful procedural \emph{case} macros. Instead of a template, the body of a case
macro is JavaScript code that runs when the macro is invoked.
Rather than be the entire body of a macro, templates are interspersed with JavaScript code by using the notation \verb!#{ ... }!.
...
Here we take the file name token matched by the macro, unwrap it, and 
...
the file into a variable. The \verb!makeValue! function is used to create a string syntax object (\verb!#{here}! is just a placeholder for the lexical context) from the context of the file and \verb!letstx! (analogous to \verb!with-syntax! in Scheme) binds that new syntax object to a pattern variable \verb!$content! that can be used inside of a template.
...
ether a divide or the start of a regular expression. Theses sets are parametrized by a boolean indicating if the prefix is inside an object literal or a block statement:
\[
\begin{array}{rrl}
\reprefix_b &::=& \epsilon
\\
...
\end{array}
\]
These sets are the prefixes used by the read function in Figure \ref{fig:simpleread}.
