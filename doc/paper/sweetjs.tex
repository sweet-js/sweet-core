...
The job of read is then to produces a correct \textit{TokenTree} sequence. Token trees include regular expression literals $\rett$ (where $x$ is the regular expression body, which we simplify here to a variable rather than include irrelevant details of parsing a regular expression body) and fully matched delimiters with nested token tree sequences $\parens{t}$ and $\curlies{t}$ rather than individual delimiters (note that we write token tree delimiters with an \underline{underline} to distinguish them from token delimiters).
...
\(
\texttt{foo} \cdot \parens{\texttt{/} \texttt{x} \texttt{/} }
\).
...
As mentioned before, Honu~\cite{Rafkind2012,Rafkind2013} is the primary inspiration for sweet.js. In contrast with Honu, which does not include regular expression literals, we solve the reader problem for JavaScript and introduce infix macros along with the \verb!invoke! pattern class.
