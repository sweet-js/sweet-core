...
The core technique that enables Scheme-like expressive macros for non
delimited languages is called \emph{enforestation} \cite{Rafkind2012}.
Enforestation works by progressively recognizing syntax forms and
expanding macros. The key idea of enforestation is to give structure
to the syntax arguments to macros.

Sweet.js implements this algorithm mostly as described with some
additions to provide infix macros and invoke pattern classes
(described in the following sections).

Enforestation works by grouping token trees into a more structured
form called a term tree. Term trees are a kind of proto-AST; each term
tree corresponds to a node in the final AST. For example,
...
After an initial pass through the enforestation process the first
binary expression will be matched as a binary term tree (we use angle
brackets to denote a term tree element).
...
Through enforestation, a macro can match structured groupings of code.
For example, consider a \lstinline!let! macro that must match the
variable initialization (which can be an arbitrary undelimited
expression):
...
The \lstinline!:expr! pattern class signals the expander to match an expression term tree, which in the case is \lstinline!<binop +, left: 40, right: 2>!.
...
We've juxtaposed the \lstinline!=>! macro next to a function call, which we did
...
tree we've built. The syntax tree would show \lstinline!bar(x)! as a complete
...
semantics, much like the \lstinline!=>! macro. Its left-hand-side and
right-hand-side are not arbitrary expressions but must match a
specific form. 
Additionally, it would need a precedence higher
...
literal. If on the other hand the previous token tree was a parenthesis
then most of the time the slash will be the divide operator (\eg
dividing the result of a call \lstinline!foo(100) / 10!). However, if
the token before the parenthesis was the keyword \texttt{if} then it
is actually the start of a regular expression (since single statement
if bodies do not require braces).
...
The main hygienic technique for procedurual macros were described in \cite{Hieb1992}. Earlier work for syntax-rules is \cite{Clinger1991}.


Mostly straightforward implementation from scheme (in for a good treatment of the subject see \cite{Flatt2012}) with some details to handle \texttt{var}.

\subsection{Var statements} 

Unfortunately there's some complexity in how \lstinline!var! (and
\lstinline!let!/\lstinline!const!) statements and macros interact. In
short, macros must be defined before being used in a var statement.
...
all other syntax forms, just not \lstinline!var!. Also Racket, from
which sweet.js takes most other design cues, allows exactly this kind
of use/definition mixing with internal definitions:
...
through each scope twice. The first pass registers any macro
definitions but critically it doesn't expand inside internal
definitions.

So after the first pass of the above example we will have something
like:
...
So far so good, but the tricky part for sweet.js is you need to know
where internal definitions end during the first pass \emph{before}
expanding any macros inside them. This is easy for Scheme since
everything is delimited but this is not the case in JavaScript.
...
When the expander gets to the \lstinline!var! statement during the
first pass it doesn't know anything about the \lstinline!=>! macro
yet. All it can know is that the right hand side of \lstinline!=!
needs to be an expression (which in this case means it will match 
\lstinline!var x = (x)! and then fail to match on \lstinline!=>!).
...
before using them in \lstinline!var! statements.

So even though we cannot defer expansion inside of \lstinline!var!
statements we still do two passes for each scope and the expansion of
any syntax inside of a delimiter is deferred until the second pass
which now has all the macro definitions in scope. Which means that you
can actually use macros in \lstinline!var! statements before
definition as long as you wrap it in parens.
