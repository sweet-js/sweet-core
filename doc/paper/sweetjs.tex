...
\item We present and prove correct a novel technique for separating
  the JavaScript lexer and parser with a \emph{reader} (section
  \ref{sec:read}).

\item We describe our implementation of hygienic macros for JavaScript
  (section \ref{sec:writingMacros}) which includes the ability to define infix macros (section \ref{sec:infix}) that match on arbitrary surrounding syntax and the \texttt{invoke} primitive (section \ref{sec:invoke}) to allow custom parser classes and more powerful matching.

\item We discuss the applicability of our reader technique for languages other than JavaScript (section \ref{sec:discussion}).
