(function (root, factory) {
    if (typeof exports === 'object') {
        // CommonJS
        factory(exports, require('underscore'), require('./parser'), require('./patterns'), require('./syntax'), require('./expander'), require('./sweet'));
...
}(this, function (exports$2, _, parser, patternModule, syntax, expander, sweet) {
    'use strict';
    ...
    MacroRule.prototype.findPatternEnvLevels = function () {
        ...
        foldReadTree(function (t, init, rest, path) {
            ...
            if (!tok)
                return;
            if (tok.type === parser.Token.Identifier && tok.class === 'token' && tok.value[0] === '$') {
                self.envLevels[tok.value] = { level: self.levelsOfPath(_(path).toArray()) };
            }
        }, this.patternRule);
    };
    MacroRule.prototype.tryMatch = function (init, rest, path, src, expanded) {
        ...
        if (!// This is necessary because of Github issue #174
            _.all(res.patternEnv, function (v) {
                return !!v.match;
            }))
            return;
