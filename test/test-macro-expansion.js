...
output = m 100 + 200`, 200);

test('should handle expansion that takes an argument', evalWithOutput, `
syntaxrec m = function(ctx) {
...
test('should handle expansion that matches an expression argument', evalWithOutput, `
syntaxrec m = function(ctx) {
...
test('should handle the macro returning an array', evalWithOutput, `
...
test('should handle the full macro context api', evalWithOutput, `
...
test('should handle iterators inside a syntax template', evalWithOutput, `
...
test('should allow the macro context to be reset', evalWithOutput, `
syntax m = ctx => {
...
test('should allow the macro context to create a reset point', evalWithOutput, `
syntax m = ctx => {
  ctx.next(); // 30
  ctx.next(); // +
  // lets play it safe
...
test('should throw if marker is from a different macro context', evalThrows, `
syntax m = ctx => {
...
test('should allow the macro context to match on a identifier expression', evalWithOutput, `
syntax m = ctx => {
...
test('should allow the macro context to match on a binary expression', evalWithOutput, `
syntax m = ctx => {
...
test('should throw an error if the match fails for MacroContext::expand', evalThrows, `
syntax m = ctx => {
  let expr = ctx.expand('BinaryExpression').value;
  return #\`\${expr}\`;
}
...
test('should construct syntax from existing syntax', evalWithOutput, `
syntax m = ctx => {
