...
const dispatchReadtable = EmptyReadtable.extendReadtable({
  key: '`',
  action: function readSyntaxTemplate(stream: CharStream, prefix: List<Syntax>, b: boolean, dispatchKey: string): List<Syntax> {
...
export default function readDispatch(stream: CharStream, prefix: List<Syntax>, b: boolean): List<Syntax> | typeof EmptyToken {
  const dispatchKey = stream.readString();
  const dispatchEntry = dispatchReadtable.getEntry(stream.peek());
  const result = dispatchEntry.action.call(this, stream, prefix, b, dispatchKey);
  return result;
}
