...
function advance() {
  ...
  if (ch === 0x60 ||
      (ch === 0x7D && state.curlyStack[state.curlyStack.length - 1] === '${')) {
...
function blockAllowed(toks, start, inExprDelim, parentIsBlock) {
  ...
  var unaryOps = ["++", "--", "~", "!", "delete", "void", "typeof", "yield", "throw",
                  "new"];
...
function readDelim(toks, inExprDelim, parentIsBlock) {
  ...
  while (index <= length) {
    ...
    if ((token.type === Token.Punctuator) &&
        (token.value === matchDelim[startDelim.value])) {
