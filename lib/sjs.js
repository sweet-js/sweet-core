...
exports.run = function () {
    ...
    var compileFile = function (infile, outfile) {
        if (sourcemap) {
            options.sourceMap = true;
            var result = sweet.compile(infile, options);
            ...
            fs.writeFileSync(outfile, sweet.compile(infile, options).code, 'utf8');
        }
    };
    if (outfile){
        compileFile(file, outfile);
        if (watch){
            timeLog("compiled " + infile);
            fs.watch(infile, function () {
                var stats = fs.statSync(infile);
                if (prevStats && stats.mtime.getTime() === prevStats.mtime.getTime()) return;
                prevStats = stats;
