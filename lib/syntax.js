...
Syntax.prototype = {
    ...
    mark: function mark(newMark) {
        if (this.token.inner) {
            this.token.inner = this.token.inner.map(function (stx$11058) {
                return stx$11058.mark(newMark);
            });
        }
        var newCtx = undefined;
    ...
    delScope: function delScope(scope) {
        if (this.token.inner) {
            this.token.inner = this.token.inner.map(function (stx$11060) {
                return stx$11060.delScope(scope);
            });
        }
        return syntaxFromToken(this.token, {
            context: this.context.filter(function (s$11061) {
                return s$11061 !== scope;
            }),
            props: this.props
        });
    },
    ...
    // non mutating
    rename: function rename(id, name, defctx, phase) {
        var // defer renaming of delimiters
        newctx = this.context == null ? [name] : this.context.concat(name);
        if (this.token.inner) {
    ...
    imported: function imported(localName, exportName, phase, mod) {
        return this;
    },
    addDefCtx: function addDefCtx(defctx) {
        return this;
    },
