...
(function (root, factory) {
    ...
    Syntax.prototype = {
        ...
        rename: function (id, name, defctx, phase) {
            ...
            return syntaxFromToken(this.token, { context: new Rename(id, name, this.context, defctx, phase) });
        },
        imported: function (id, name, phase) {
            if (this.token.inner) {
                return syntaxFromToken(this.token, {
                    deferredContext: new Imported(id, name, this.deferredContext, phase),
                    context: new Imported(id, name, this.context, phase)
                });
            }
            return syntaxFromToken(this.token, { context: new Imported(id, name, this.context, phase) });
        },
        ...
        expose: function () {
            ...
            function applyContext(stxCtx, ctx) {
                if (ctx == null) {
                    ...
                    return new Rename(ctx.id, ctx.name, applyContext(stxCtx, ctx.context), ctx.def, ctx.phase);
                    ...
                    return new Imported(ctx.id, ctx.name, applyContext(stxCtx, ctx.context), ctx.phase);
                } else {
