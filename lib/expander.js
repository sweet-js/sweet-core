(function (root, factory) {
    ...
    function expandToTermTree(stx, context) {
        ...
        while (rest.length > 0) {
            ...
            if (head.isImport) {
                ...
                importMod = loadImport(head, context);
    ...
    function createModule(name, body) {
        ...
        if (body && body[0] && body[1] && body[2] && unwrapSyntax(body[0]) === '#' && unwrapSyntax(body[1]) === 'lang' && body[2].token.type === parser.Token.StringLiteral) {
            ...
            modBody = body[3] && body[3].token.value === ';' && body[3].token.type == parser.Token.Punctuator ? body.slice(4) : body.slice(3);
        }
    ...
    function invoke(modTerm, modRecord, phase, context) {
        if (modRecord.language === 'base') {
            ...
            modRecord.importEntries.forEach(function (imp) {
            ...
            modRecord.exportEntries.forEach(function (exp) {
    ...
    function visit(modTerm, modRecord, phase, context) {
        ...
        modRecord.importEntries.forEach(function (imp) {
            ...
            modTerm.body = bindImportInMod(imp, modTerm.body, importMod.term, importMod.record, context, phase);
        });
                ...
                if (term.name.token.type === parser.Token.Delimiter && term.name.token.value === '{}') {
                    (function (names) {
                        return names.forEach(function (name$2) {
                            modRecord.exportEntries.push(name$2);
                        });
    ...
    function loadImport(imp, context) {
        ...
        if (!availableModules.has(modFullPath)) {
            ...
            modToImport = function (mod) {
                return expandModule(mod.term, modFullPath, context.templateMap, context.patternMap, mod.record);
    ...
    function bindImportInMod(imp, stx, modTerm, modRecord, context, phase) {
        if (// todo: implement other import forms
            imp.names.token.type !== parser.Token.Delimiter) {
            assert(false, 'not implemented yet');
        }
        ...
        renamedNames = function (names) {
            return names.map(function (importName) {
                ...
                var inExports = _.find(modRecord.exportEntries, function (expTerm) {
    ...
    function expandModule(mod, filename, templateMap, patternMap, moduleRecord) {
        ...
        context = makeModuleExpanderContext(filename, templateMap, patternMap, 0, moduleRecord);
        return {
            context: context,
    ...
    function flattenModule(modTerm, modRecord, context) {
        ...
        imports = modRecord.importEntries.reduce(function (acc, imp) {
        ...
        output = modTerm.body.reduce(function (acc, term) {
            ...
            return acc.concat(term.destruct(context, {
                stripCompileTerm: true,
                stripModuleTerm: true
            }));
        }, []);
        ...
        flatImports = imports.reduce(function (acc, imp) {
    ...
    exports$2.compileModule = compileModule;
