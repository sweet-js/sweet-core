'use strict';
var _ = require('underscore'), assert = require('assert'), unwrapSyntax = require('../syntax').unwrapSyntax, makeIdent = require('../syntax').makeIdent, resolve = require('../stx/resolve').resolve, StringMap = require('./stringMap');
function NameMap() {
...
NameMap.prototype.setWithModule = function (stx, phase, value, module$2) {
    this.set(stx, phase, value);
    if (this._modules.has(module$2)) {
        this._modules.get(module$2).set(resolve(stx, phase), value);
    } else {
        this._modules.set(module$2, new StringMap());
    }
};
