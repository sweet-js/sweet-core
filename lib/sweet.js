...
(function (exports) {
    ...
    var tokensToSyntax = guard(fun(Any, Any), function tokensToSyntax(tokens) {
            if (!_.isArray(tokens)) {
                tokens = [
                    tokens
                ];
    ...
    var syntaxToTokens = guard(fun(Any, Any), function syntaxToTokens(syntax) {
    ...
    var isPatternVar = guard(fun(Any, Any), function isPatternVar(token) {
            return token.type === Token.Identifier && token.value[0] === '$';
        });
    var mkPatternArray = guard(fun(Any, Any), function loadPattern(patterns) {
            var result = [];
            var index = 0;
    ...
    var matchPatterns = guard(fun(Any, Any), function matchPatterns(callSyntax, patterns) {
            var matches = {};
            var callIdx = 0;
            var parseResult;
            var syntax = [].concat(callSyntax, tokensToSyntax({
                    type: Token.EOF
                }));
            patterns.forEach(function (pattern) {
                if (pattern.class === 'pattern_literal') {
                    ...
                    tokInner = [].concat(tokInner, tokensToSyntax({
                        type: Token.EOF
                    }));
                    parseResult = parse_stx(syntaxToTokens(tokInner), patInner.class, {
                        tokens: true
                    }).tokens;
                    matches[patInner.value] = tokensToSyntax(parseResult);
                } else {
                    var parseResult = parse_stx(syntaxToTokens(_.rest(syntax, callIdx)), pattern.class, {
                            tokens: true
                        });
    ...
    var takeContext = guard(fun([
            Any,
            Any
        ], Any), function takeContext(from, to) {
    ...
    var mkMacroTransformer = guard(fun(Any, Any), function mkMacroTransformer(patternSyntax, bodySyntax) {
    ...
    var loadMacro = guard(fun(Any, Any), function loadMacro(macroBody) {
            assert(macroBody[0].token.value === 'case', 'case pattern does not have `case` keyword');
            assert(macroBody[2].token.value === '=' && macroBody[3].token.value === '>', 'case pattern does not have `=>` token');
    ...
    var flatten = guard(fun(Any, Any), function flatten(tokens) {
            var flat = [];
