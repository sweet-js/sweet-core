...
(function (exports) {
    ...
    var matchPatterns = guard(fun([
            ...
            CPatternStream
        ], Any), function matchPatterns(callTokens, patterns) {
            var matches = {};
            ...
            patterns.forEach(function (pattern) {
                if (pattern.type === 'pattern_literal') {
                    ...
                    tmp = parse_stx(tokInner, patInner.type, {
                        tokens: true
                    }).tokens;
                    matches[patInner.name] = tmp;
                } else {
                    var tmp = parse_stx(stream.rest(), pattern.type, {
                            tokens: true
                        });
    ...
    var takeContext = guard(fun([
            ...
            CToken
        ], Any), function takeContext(to, from) {
    ...
    var invokeMacro = guard(fun([
            ...
            CMacroDef
        ], Any), function invokeMacro(callArgs, macroDefinition) {
            ...
            return expand(Macros.tokensToSyntax(res));
        });
    function expand(stx, macros) {
        ...
        while (index < stx.length) {
            ...
            if (token.type === Token.Identifier && token.value === 'macro') {
                ...
                expanded = expanded.concat(expand(Macros.tokensToSyntax(token.inner), macros));
                expanded.push({
                    type: Token.Punctuator,
                    value: token.value[1],
    ...
    function read(code) {
        ...
        return Macros.tokensToSyntax(tokenTree);
    }
