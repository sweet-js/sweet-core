...
(function (exports) {
    ...
    function matchAssign() {
        var token = lookahead().token, op = token.value;
    ...
    function parsePrimaryExpression() {
        var expr, token = lookahead().token, type = token.type;
    ...
    function parseStatement() {
        var token = lookahead().token, expr, labeledBody;
        if (token.type === Token.EOF) {
            throwUnexpected(token);
        }
        if (token.type === Token.Punctuator) {
    ...
    var syntaxProto = {
            ...
            rename: function (idents, name) {
                ...
                var ids = _.isArray(idents) ? idents : [
                        idents
                    ];
            ...
            swap_dummy_rename: function (ident, name, dummyName) {
                ...
                return syntaxFromToken(swappedToken, renameDummyCtx(this.context, ident, name, dummyName));
            }
        };
    ...
    function isPatternVar(token) {
        return token.type === Token.Identifier && token.value[0] === '$' && token.value !== '$';
    }
    ...
    function loadPattern(patterns) {
        return _.chain(patterns).reduce(function (acc, patStx, idx) {
            ...
            if (patStx.token.value === '...' || delimIsSeparator(patStx) && next && next.token.value === '...') {
                return acc;
            }
    ...
    function matchPatterns(callStx, patterns) {
        ...
        var callIdx = 0;
        var parseResult;
        var syntax = [].concat(callStx, tokensToSyntax({
                type: Token.EOF
            }));
            ...
            if (pattern.class === 'pattern_literal' && pattern.token.type !== Token.Delimiter) {
                ...
                do {
                    if (pattern.token.type === Token.Delimiter) {
                        ...
                        parseResult = parse_stx(_.rest(syntax, callIdx), pattern.class, {
                            tokens: true
                        });
    ...
    function delimIsSeparator(delim) {
        return delim && delim.token.type === Token.Delimiter && delim.token.value === '()' && delim.token.inner.length === 1 && delim.token.inner[0].token.type !== Token.Delimiter && !containsPatternVar(delim.token.inner);
    }
    function mkMacroTransformer(macroCases) {
        ...
        return function (callSyntax, macroNameStx) {
            var matches = _.chain(_.zip(callSyntax, patterns)).map(function (ziped) {
                    var call = ziped[0], pat = ziped[1];
            ...
            var substitute = function (toSubstitute) {
                return _.chain(toSubstitute).reduce(function (acc, bodyStx, idx, original) {
    ...
    function getArgList(argSyntax) {
        assert(argSyntax.token.type === Token.Delimiter, 'expecting delimiter for function params');
        return _.filter(argSyntax.token.inner, function (stx) {
            return stx.token.value !== ',';
        });
    }
    function isFunctionStx(stx) {
        return stx && stx.token.type === Token.Keyword && stx.token.value === 'function';
    }
    function isVarStx(stx) {
        return stx && stx.token.type === Token.Keyword && stx.token.value === 'var';
    }
    ...
    function getVarIdentifiers(body) {
        return _.reduce(body, function (acc, curr, idx) {
            ...
            if (curr.token.type === Token.Delimiter) {
                atFunctionDelimiter = curr.token.value === '()' && (isFunctionStx(body[idx - 1]) || isFunctionStx(body[idx - 2])) || curr.token.value === '{}' && (isFunctionStx(body[idx - 2]) || isFunctionStx(body[idx - 3]));
                ...
                var parseResult = parse_stx(flatten(body.slice(idx)), 'varDeclarationList', {
                        noresolve: true
                    });
        ...
        while (index < stx.length) {
            ...
            if (token.type === Token.Identifier && token.value === 'macro') {
                ...
                var callArgs = _.map(_.range(macroDef.toConsume), function () {
                        assert(stx[index].token.type === Token.Delimiter, 'expecting delimiters in macro call');
                        return stx[index++];
                    });
                var newMark = fresh();
                var markedArgs = _.map(callArgs, function (arg) {
                        return arg.mark(newMark);
                    });
                var macResult = macros[token.value].transformer(markedArgs, currStx);
                var markedResult = _.map(macResult, function (arg) {
                        return arg.mark(newMark);
                    });
